...
9. During an approved apply, installer/backup/manual rollback share one lock. A running baseline must pass parsed `openclaw health --json` RPC before known-good pointers change. The archive must validate before pointer replacement. If the service began stopped, it stays stopped and existing known-good pointers are preserved.
10. Any ERR/INT/TERM/unexpected EXIT after mutation starts triggers phase-aware recovery of the exact prior runtime/path and prior running/stopped state. Success is never printed before path and required structured-health verification. On any recovery warning or failure, stop and follow the owner recovery route—never perform package surgery.
